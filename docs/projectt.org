...
- corpus test on Bible AND Wikipedias is clean from @# errors and gives >95%
  coverage
- apertium-html-tools localised into the languages the student worked on
...
- bonus point: Common Voice's interface translated into kaa uig uzb (using our
...
- add 10 vocabulary tests (testing soviet dictionary entry words) to kaa, uzb,
  tat, kaz, kir, uig, tur
